...
define users::virtual::account(
  ...
  $ssh_key = undef, $ssh_key_type = 'ssh-rsa', $groups = [], $ensure = present,$purge_ssh_keys=true,
  ...
  if $create_group == true {
    ...
    case $ensure {
      ...
      default: {
        fail ('ensure is eitehr abesent or present')
      }
    }
  }
  else {
  ...
  case $ensure {
    present: {
      ...
      if $ssh_key != undef {
        $key_default = {
          require => File["${home_dir_real}/.ssh"]
        }
        create_resources('ssh_authorized_key', $ssh_key,$key_default)
      }
    }
    absent: {
      $dir_ensure = absent
